...
from husky.configuration_husky import HuskyConfig, HuskyQFormerConfig, HuskyVisionConfig
...
class HuskyPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, HuskyVisionEmbeddings) or isinstance(module, HuskyVideoEmbeddings):
...
Husky_VISION_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            Pixel values. Pixel values can be obtained using [`Husky2Processor`]. See [`Husky2Processor.__call__`] for
            details.
        output_attentions (`bool`, *optional*):
...
Husky_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            Indices can be obtained using [`Husky2Processor`]. See [`Husky2Processor.__call__`] for details.
...
class HuskyQFormerLayer(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.intermediate = HuskyQFormerIntermediate(config)
        self.output = HuskyQFormerOutput(config)
...
class HuskyQFormerEmbeddings(nn.Module):
...
class HuskyQFormerModel(HuskyPreTrainedModel):
    """
    Querying Transformer (Q-Former), used in Husky. Slightly modified from BLIP-2 as it also takes the
    ...
    def __init__(self, config: HuskyQFormerConfig):
        ...
        self.embeddings = HuskyQFormerEmbeddings(config)
...
@add_start_docstrings(
    """
    Husky Model for generating text and image features. The model consists of a vision encoder, Querying
...
class HuskyModel(HuskyPreTrainedModel):
    ...
    def __init__(self, config: HuskyConfig):
        ...
        self.vision_adapter = AdapterMLP(config.vision_config)
    ...
    def get_qformer_features(
            ...
            pixel_values: Optional[torch.FloatTensor] = None,
            vision_embeds: Optional[torch.FloatTensor] = None,
            qformer_input_ids: Optional[torch.FloatTensor] = None,
        ...
        # TODO: difference with BLIP-2 here: we also feed the instruction prompt to the Q-Former
    ...
    @add_start_docstrings_to_model_forward(Husky_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=HuskyForConditionalGenerationModelOutput, config_class=HuskyVisionConfig
    )
    def forward(
            self,
            pixel_values: Optional[torch.FloatTensor] = None,
...
class HuskyForConditionalGeneration(HuskyPreTrainedModel):
    ...
    def generate(
        ...
        Args:
            ...
            vision_embeds (`torch.FloatTensor` of shape (batch_size, num_channels, sequence_length)):
                Input images embeddings.
        ...
        with self.maybe_autocast():
            image_embeds = self.vision_model(pixel_values, vision_embeds, return_dict=True).last_hidden_state
        image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device)
