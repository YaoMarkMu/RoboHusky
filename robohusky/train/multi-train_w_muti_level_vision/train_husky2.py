...
def main():
    ...
    llm_path = "/mnt/petrelfs/zhangqinglong/Documents/Husky/work_dirs/llm/husky-13b/zh_belle_share_gpt4_twitter"
    ...
    image_embeddings = model.vision_model.embeddings
    video_embeddings = model.vision_model.video_embeddings
    video_embeddings.class_embedding = image_embeddings.class_embedding
    position_embedding = resize_pos_embed(
        image_embeddings.position_embedding,
        video_embeddings.position_embedding
    )
    video_embeddings.position_embedding.data = position_embedding.data
    temporal_size = video_embeddings.patch_embedding.weight.shape[2]
    patch_embedding_weight = image_embeddings.patch_embedding.weight.unsqueeze(2).repeat(
        1, 1, temporal_size, 1, 1) / temporal_size

    video_embeddings.patch_embedding.weight.data = patch_embedding_weight.data
    model.vision_model.video_embeddings = video_embeddings
    """
    ...
    mmc4_130k_zh = mmc4_130k_zh.select(range(0, 80000))
    coco_5k_zh = load_from_disk("datasets/tokenized/chat_coco_zh")
    coco_5k_zh = datasets.concatenate_datasets([coco_5k_zh, coco_5k_zh, coco_5k_zh, coco_5k_zh])
    chinese_food = load_from_disk("datasets/tokenized/ChineseFood")
    chinese_food = datasets.concatenate_datasets([chinese_food, chinese_food])
    zh_dataset = datasets.concatenate_datasets([cc3m_595k_zh, coco_5k_zh, mmc4_130k_zh, chinese_food])
    ...
    # load video dataset
    webvid2m = load_from_disk("datasets/tokenized/webvid2m")
    # max_webvid = 512 * (len(webvid2m) // 512)
    # webvid2m = webvid2m.select(range(max_webvid))
    youku = load_from_disk("datasets/tokenized/Youku")
    # ego_4d = load_from_disk("datasets/tokenized/EGO4D")
    # ego_yf = load_from_disk("datasets/tokenized/EGOHYF")
    # ego_yf = datasets.concatenate_datasets([ego_yf, ego_yf, ego_yf, ego_yf])
    # ego_web = load_from_disk("datasets/tokenized/webvid2m_ego")
    # video_dataset = datasets.concatenate_datasets([ego_4d, ego_yf, ego_web])
    video_dataset = datasets.concatenate_datasets([webvid2m, youku, youku])
