...
def main():
    ...
    send_example_telemetry("zh_husky", model_args, data_args)
    ...
    if data_args.conv_style is not None:
        tokens_list = [DEFAULT_IMG_START_TOKEN, DEFAULT_IMG_END_TOKEN, DEFAULT_VIDEO_START_TOKEN,
    ...
    cc_sbu_align = load_from_disk("datasets/tokenized/cc_sbu_5k")
    cc_sbu_align = datasets.concatenate_datasets([cc_sbu_align, cc_sbu_align])
    llava_150k = load_from_disk("datasets/tokenized/llava_150k")
    llava_w_loc = load_from_disk("datasets/tokenized/llava_w_loc_150k")
    vqa2 = load_from_disk("datasets/tokenized/VQAv2")
    shira_cot = load_from_disk("datasets/tokenized/ShikraCot")
    shira_rd = load_from_disk("datasets/tokenized/Shikra_RDCot")
    paragraph_caps = load_from_disk("datasets/tokenized/image_paragraph_caps")
    en_dataset = datasets.concatenate_datasets(
        [cc_sbu_align, llava_150k, llava_w_loc, vqa2, shira_cot, shira_rd, paragraph_caps])

    en_dataset = datasets.concatenate_datasets([en_dataset, en_dataset, en_dataset, en_dataset])
    en_dataset = en_dataset.shuffle(seed=42)
    svit = load_from_disk("datasets/tokenized/SVIT")
    en_dataset = datasets.concatenate_datasets([en_dataset, svit])

    # Chinese image dataset
    vqa2_zh = load_from_disk("datasets/tokenized/VQAv2_zh")
    llava_zh_100k = load_from_disk("datasets/tokenized/llava_100k_zh")
    china_poetry = load_from_disk("datasets/tokenized/ChinaPoetry")
    zh_dataset = datasets.concatenate_datasets([llava_zh_100k, vqa2_zh, china_poetry])
    zh_dataset = datasets.concatenate_datasets([zh_dataset, zh_dataset, zh_dataset, zh_dataset])
    zh_dataset = datasets.concatenate_datasets([zh_dataset, zh_dataset])
    zh_dataset = zh_dataset.shuffle(seed=42)

    # concat english and chinese dataset
    image_dataset = datasets.concatenate_datasets([en_dataset, zh_dataset])

    # here we assume total training batch_size is 256
    # max_image_sample = int(len(image_dataset) // 256) * 256
    # image_dataset = image_dataset.select(range(0, max_image_sample))
    train_dataset = image_dataset

    # train_dataset = datasets.concatenate_datasets([cc_sbu_align, llava_150k, llava_zh_100k, cc_sbu_align])
    # load video dataset
    # webvid2m_en = load_from_disk("datasets/tokenized/webvid2m_en")
    # webvid2m_zh = load_from_disk("datasets/tokenized/webvid2m_zh")
    # ego_my = load_from_disk("datasets/tokenized/EGOMY")
    # ego_my = datasets.concatenate_datasets([ego_my, ego_my, ego_my, ego_my])
    # video_dataset = datasets.concatenate_datasets([ego_my, ego_my, webvid2m_zh, webvid2m_en])
    # video_dataset = video_dataset.shuffle(seed=42).shuffle(seed=0)

    # video_dataset = datasets.concatenate_datasets([webvid2m_en, webvid2m_zh])
    # concat image and video dataset
    # train_dataset = datasets.concatenate_datasets([image_dataset, video_dataset])
