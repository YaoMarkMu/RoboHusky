...
class HuskyForConditionalGeneration(HuskyPreTrainedModel):
    ...
    def __init__(self, config: HuskyConfig):
        ...
        self.vision_layer_norms = [nn.LayerNorm(config.vision_config.hidden_size, eps=1e-6) for _ in range(4)]
    ...
    def forward(
        ...
        breakpoint()
        pooled_outputs = [vision_outputs[2][indices[idx][:, 0, :]] for idx in indices]
        breakpoint()
        pooled_outputs = [self.vision_layer_norms[i][pooled_outputs[i]] for i in range(4)]
        breakpoint()
